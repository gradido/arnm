...
pub fn build(b: *std.Build) void {
    ...
    if (enable_benchmarks) {
        ...
        processBuildTarget(&context, .{ .name = "bench_graded_block_pool", .srcs = &.{"bench_graded_block_pool.c"} }, path);
    }
    ...
    if (enable_tests) {
        ...
        processBuildTarget(&context, .{ .link_googletest = true, .name = "test_graded_block_pool", .srcs = &.{"test_graded_block_pool.cpp"} }, path);
        ...
        processBuildTarget(&context, .{ .link_googletest = true, .name = "test_roaring_ops", .srcs = &.{"test_roaring_ops.cpp"} }, path);
        processBuildTarget(&context, .{ .link_googletest = true, .name = "test_roaring_query", .srcs = &.{"test_roaring_query.cpp"} }, path);
