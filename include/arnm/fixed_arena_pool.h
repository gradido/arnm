#ifndef ARNM_FIXED_ARENA_POOL_H
#define ARNM_FIXED_ARENA_POOL_H
...
/**
 * @defgroup arnm_fixed_arena_pool arnm_fixed_arena_pool
 * @brief A fixed set of equal sized arenas, lent out one at a time.
 *
 * Where @ref arnm_multi_arena grows until the host runs out, this one does not grow at all.
 * @ref arnm_fixed_arena_pool_init() reserves every arena it will ever have, in a single block,
 ...
 * ### Ownership
 *
 * The pool owns exactly the arenas that are free. @ref arnm_fixed_arena_pool_alloc() hands one
 * over and forgets it; while the caller holds it the pool knows nothing about it and tracks
 * nothing but a count. @ref arnm_fixed_arena_pool_free() takes it back, resetting it on the
 * way in, so an arena always arrives empty.
 *
 * No handles, no busy list, no owner bookkeeping. What a caller holds is an ordinary
 * @ref arnm, usable with every function in @ref arnm_memory -- the pool is where it came
 ...
 *
 * `_release` refuses with @ref ARNM_ERROR_RESOURCE_IN_USE while anything is still out. It
 ...
 * _release(), the way a size is given to arnm_alloc() and again to arnm_free(). Which
 ...
 *
 * arnm_fixed_arena_pool_init() writes every field and reads none, so uninitialized storage is
 * a valid input. Unlike @ref arnm there is no useful zero state: a pool that owns nothing can
 * hand nothing out, and every call says so rather than pretending.
 *
 * @note Do not write these fields; read them through the API.
 */
typedef struct arnm_fixed_arena_pool {
  /**
 ...
 * this pool has: either every arena is there when this returns ARNM_SUCCESS, or nothing was
 ...
 * @retval ARNM_SUCCESS                   Every arena is reserved and free.
 * @retval ARNM_ERROR_NULL_POINTER        @p pool is NULL.
 * @retval ARNM_ERROR_INVALID_PARAM       @p arena_capacity or @p arena_count is 0.
 * @retval ARNM_ERROR_ARITHMETIC_OVERFLOW The block would not fit
 *                                           @ref ARNM_MAX_ALLOC_SIZE.
 * @retval ARNM_ERROR_OUT_OF_MEMORY       @p source had no room for the whole block. Nothing
 *                                           partial is kept.
 * @warning Calling this on a pool that already holds arenas leaks the old block. Use
 *          arnm_fixed_arena_pool_release() first.
 * @warning @p source is not remembered. arnm_fixed_arena_pool_release() has to be handed the
 ...
 *          the address check in arnm_free() and only costs the block, which stays with its
 ...
 *
 * @param[in]     arena_capacity As in arnm_fixed_arena_pool_init().
 * @param[in]     arena_count    As in arnm_fixed_arena_pool_init().
 * @param[in,out] source         As in arnm_fixed_arena_pool_init(): for the arenas.
 ...
 *         arnm_fixed_arena_pool_init() refused the arguments.
 * @note Pair with arnm_fixed_arena_pool_destroy() **and hand it the same @p allocator**.
 ...
 * @retval ARNM_ERROR_NOT_INITIALIZED     @p pool holds no arenas; it was never initialized,
 *                                           or has been released.
 * @retval ARNM_ERROR_RESOURCE_EXHAUSTED  Every arena is out with someone. Nothing is wrong
 *                                           with the request; the pool is simply the size it is.
 * @note Never arnm_release() an arena from here. Its buffer belongs to the pool's block, and
 *       arnm_fixed_arena_pool_free() is how it comes back.
 ...
 * @param[in,out] arena Arena from arnm_fixed_arena_pool_alloc(); not NULL.
 * @retval ARNM_SUCCESS               Reset and back on the free list.
 * @retval ARNM_ERROR_NULL_POINTER    @p pool or @p arena is NULL.
 * @retval ARNM_ERROR_NOT_INITIALIZED @p pool holds no arenas.
 * @retval ARNM_ERROR_INVALID_PARAM   @p arena is not one of this pool's arenas.
 ...
 *                       arnm_fixed_arena_pool_init() was handed, NULL for malloc.
 * @retval ARNM_SUCCESS               Block returned, pool empty.
 * @retval ARNM_ERROR_NULL_POINTER    @p pool is NULL.
 * @retval ARNM_ERROR_RESOURCE_IN_USE At least one arena is still out. Nothing was changed;
 *                                       return them and call again.
 * @retval ARNM_WARNING_ARENA_MEMORY_NOT_RECLAIMED @p source is an arena and the block is not
...
/**
 * @brief arnm_fixed_arena_pool_release(), then give the descriptor itself back.
 *
 * @param[in,out] pool      From arnm_fixed_arena_pool_create(), never stack or static
 *                          storage; may be NULL.
 * @param[in,out] source     The allocator the arenas came from, passed straight to
 *                           arnm_fixed_arena_pool_release(), NULL for malloc.
 * @param[in,out] allocator The allocator @p pool itself came from -- the same one
 *                          arnm_fixed_arena_pool_create() was handed, NULL for malloc. The
 *                          two are separate and are usually not the same allocator.
 * @retval ARNM_SUCCESS               Released, or @p pool was NULL and there was nothing
 *                                       to do.
 ...
 * @retval ARNM_WARNING_ARENA_MEMORY_NOT_RECLAIMED An arena kept the block or the descriptor.
 ...
 */
static inline uint16_t arnm_fixed_arena_pool_available(const arnm_fixed_arena_pool *pool) {
