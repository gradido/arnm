...
/** @brief Strict RFC 8259: no comments, no trailing commas, no inf, no nan. */
...
/** @brief Accept one trailing comma per array or object -- `[1,2,3,]`. Non-standard. */
#define ARNM_JSON_READ_ALLOW_TRAILING_COMMAS ((arnm_json_read_flags)(1u << 1))
/** @brief Accept `//` and the block form as whitespace. Non-standard. */
#define ARNM_JSON_READ_ALLOW_COMMENTS ((arnm_json_read_flags)(1u << 2))
/** @brief Accept `inf`, `-Infinity`, `nan` and numbers that overflow to them. Non-standard. */
#define ARNM_JSON_READ_ALLOW_INF_AND_NAN ((arnm_json_read_flags)(1u << 3))
/** @brief Carry malformed UTF-8 through instead of refusing it. Non-standard. */
#define ARNM_JSON_READ_ALLOW_INVALID_UNICODE ((arnm_json_read_flags)(1u << 4))
/** @brief Skip a UTF-8 byte order mark ahead of the document. Non-standard. */
#define ARNM_JSON_READ_ALLOW_BOM ((arnm_json_read_flags)(1u << 5))
 ...
 * @retval ARNM_ERROR_NULL_POINTER      @p value or @p out is NULL.
 * @retval ARNM_ERROR_INVALID_ENUM_TYPE The value is not a number.
 * @note Only @ref ARNM_JSON_READ_ALLOW_INF_AND_NAN lets a non-finite value reach this at all.
 */
