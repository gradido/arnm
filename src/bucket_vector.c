...
static inline uint32_t bucket_not_empty_count(const arnm_bvec *v) {
...
static inline bool is_state_valid(arnm_bvec *v) {
  // empty buckets
  if (!v->buckets) {
    if (v->bucket_count || v->bucket_capacity || v->tail_index || v->tail || v->size) {
      return false;
    }
    return true;
  } else {
  ...
  if (v->bucket_count > v->bucket_capacity) return false;
  if (v->tail_index >= v->bucket_count) return false;
  if (!v->tail && v->tail_index) return false;
  if (v->size > v->bucket_count * bucket_capacity(v->bucket_capacity_max_log2)) return false;
...
arnm_result arnm_bvec_init(arnm_bvec *v, uint8_t bucket_capacity_log2, uint8_t index_grow_step_size, size_t type_size, arnm *allocator)
{
  ...
  if (!type_size || !bucket_capacity_log2 || bucket_capacity_log2 > 16 || type_size > (size_t)(UINT32_MAX >> (bucket_capacity_log2)))
  ...
  v->index_grow_step_size = index_grow_step_size ? index_grow_step_size : ARNM_BVEC_DEFAULT_INDEX_GROW_STEP_SIZE;
...
arnm_result arnm_bvec_reserve(
  ...
  if (!v) return ARNM_ERROR_NULL_POINTER;
  if (!element_count || !type_size) return ARNM_ERROR_INVALID_PARAM;
  ...
  if (element_count > (UINT32_MAX - bucket_mask(v->bucket_capacity_max_log2)) / type_size) {
    return ARNM_ERROR_ARITHMETIC_OVERFLOW;
  }
  uint16_t needed = (element_count + bucket_mask(v->bucket_capacity_max_log2)) >> v->bucket_capacity_max_log2;
  if (needed <= v->bucket_count) return ARNM_SUCCESS;
  if (needed > v->bucket_capacity) {
    ...
    if (new_capacity > ARNM_BVEC_MAX_INDEX_CAPACITY) {
      return ARNM_ERROR_ARITHMETIC_OVERFLOW;
    }
    arnm_result result = arnm_realloc(
        (uint8_t**)v->buckets, old_capacity, new_capacity, v->allocator
    );
  ...
  uint16_t previous_bucket_count = v->bucket_count;
  const uint32_t bucket_size = bucket_bytes(type_size, v->bucket_capacity_max_log2);
...
arnm_result arnm_bvec_shrink(arnm_bvec *v, size_t type_size) {
  ...
  if (!type_size) return ARNM_ERROR_INVALID_PARAM;
  if (!is_state_valid(v)) return ARNM_ERROR_INVALID_STATE;

  const uint32_t bucket_size = bucket_bytes(type_size, v->bucket_capacity_max_log2);
  const uint32_t used_before = bucket_not_empty_count(v);
  ...
  if (!i) {
    ...
    /* of stranding it                                                                 */
    if (ARNM_SUCCESS == arnm_free((uint8_t *)v->buckets, index_bytes(v->bucket_capacity), v->allocator)) {
      v->buckets = NULL;
      v->bucket_capacity = 0;
    }
    return ARNM_SUCCESS;
...
{
  if (!v || !v->buckets || !type_size) return;
  const uint32_t bucket_size = bucket_bytes(type_size, v->bucket_capacity_max_log2);
  ...
  (void)arnm_bvec_init(v, v->bucket_capacity_max_log2, v->index_grow_step_size, type_size, v->allocator);
}
...
arnm_result arnm_bvec_grow(arnm_bvec *v, size_t type_size, void **out_slot) {
  ...
  uint32_t next = bucket_not_empty_count(v);
  if (next >= v->bucket_count) {
    if (v->bucket_count == v->bucket_capacity) {
      ...
      arnm_result result = arnm_realloc(
          (uint8_t **)index, index_bytes(v->bucket_capacity), index_bytes(new_capacity), v->allocator
      );
      if (result != ARNM_WARNING_ARENA_MEMORY_NOT_RECLAIMED && result != ARNM_SUCCESS) return result;
      v->bucket_capacity = new_capacity;
    }
    // if (ARNM_SUCCESS != arnm_alloc(&buffer, size, allocator)) return NULL;
    void *bucket = NULL;
    arnm_result result = arnm_alloc((uint8_t**)&bucket, bucket_bytes(type_size, v->bucket_capacity_max_log2), v->allocator);
...
arnm_result arnm_bvec_emplace(arnm_bvec *v, size_t type_size, void **out_slot) {
  ...
  return arnm_bvec_grow(v, type_size, out_slot);
}

arnm_result arnm_bvec_push_ptr(arnm_bvec *v, size_t type_size, const void *value) {
  void *slot;
  arnm_result result = arnm_bvec_emplace(v, type_size, &slot);
  if (result != ARNM_SUCCESS) return result;
  memcpy(slot, value, type_size);
  return ARNM_SUCCESS;
}
...
arnm_result arnm_bvec_copy_to(const arnm_bvec *v, size_t type_size, void *dst, uint32_t dst_capacity) {
  ...
  for (bucket = 0; bucket < buckets; ++bucket) {
    ...
    memcpy(dst + written, v->buckets[bucket], count * type_size);
    written += count;
  }
  return ARNM_SUCCESS;
}
