...
| `arnm/roaring_bitmap.h` | compressed set of `uint32_t` values added in ascending order; sorted array while small, roaring containers once it grows, blocks from a graded block pool |
| `arnm/roaring_ops.h` | and, or, andnot and a range copied out of such sets, each into a set you provide |
| `arnm/roaring_query.h` | how many values several such sets match, and which page of them -- in all of these, one of those, none of these, inside a range -- without building a result
...
```bash
zig fetch --save git+https://github.com/gradido/arnm#v0.8.3
```
